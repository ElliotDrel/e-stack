...
Run all five. CI (`.github/workflows/publish.yml`) runs gates 1, 3, 4, and 5 as hard blocks on the release — anything missed here fails the publish later. Gate 2 is a repo convention, not a CI check.
...
5. `pytest -q`, then the migrate smoke tests (exact commands in `.github/workflows/publish.yml`) — the test suite must pass.
