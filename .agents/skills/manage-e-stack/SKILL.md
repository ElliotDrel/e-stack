...
- **`description` field** MUST start with `(<short-name>)` where short-name is the folder name without the `estack-` prefix — e.g. `(chris-voss) Applies...` Write it as a `>-` folded block, never a double-quoted string: `scripts/check-skill-name.cjs` reads the raw value and a leading `"` fails the prefix check (hit 2026-09-05). Keep it to one to three sentences that name the moment the skill is needed, not a topic list or a run of trigger phrases.
